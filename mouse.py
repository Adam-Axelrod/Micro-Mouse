...
class MouseState:
    ...
    def step(self, left_wheel_speed_mms, right_wheel_speed_mms, delta_time_seconds):
        ...
        self.left_encoder_ticks += round((left_wheel_speed_mms * delta_time_seconds) / config.MM_PER_TICK)
        self.right_encoder_ticks += round((right_wheel_speed_mms * delta_time_seconds) / config.MM_PER_TICK)
    ...
    def sensor_position_and_angle(self, sensor_name):
        ...
        if sensor_name == "left":
            local_offset_x = -25.0
            local_offset_y = 45.0
            relative_angle = math.pi / 4.0   # +45 degrees
    ...
    def read_sensor_adc(self, sensor_name, wall_segments):
        ...
        distance_mm = cast_ray(origin, angle, wall_segments, max_range_mm=250.0)

        # Phototransistor reflectance model: intensity = K / (d + d0)^2
        if distance_mm >= 240.0:
            return 200  # Background noise floor

        scaling_factor = 4.5e7
        distance_offset = 15.0
        intensity = scaling_factor / ((distance_mm + distance_offset) ** 2)
        adc_value = int(min(max(intensity, 200.0), 65535.0))
        return adc_value
